...
@section('content')
    ...
    <!-- Order yang Sudah Di-approve oleh Umum (Perlu Validasi Final) -->
    <div style="background: white; padding: 25px; border-radius: 8px; margin-bottom: 30px; border: 2px solid var(--accent);">
        <h3 style="font-size: 20px; font-weight: bold; margin-bottom: 20px; color: #333; padding-bottom: 10px; border-bottom: 2px solid var(--accent);">
            Order yang Perlu Validasi Final (Sudah Di-approve oleh Umum)
        </h3>
        
        @if($orderApprovedByUmum->count() > 0)
        <div style="overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    @foreach($orderApprovedByUmum as $order)
                    <tr style="border-bottom: 1px solid #eee;">
                        ...
                        <td style="padding: 12px; text-align: center; border: 1px solid #eee;">
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                <form action="{{ route('order.validate-keuangan', $order->id_order) }}" method="POST" style="display: inline;">
                                    @csrf
                                    <input type="hidden" name="action" value="approve">
                                    <button type="submit" 
                                            onclick="return confirm('Apakah Anda yakin ingin menyetujui final order ini?')"
        ...
        <div style="padding: 40px; text-align: center; color: #999; border: 1px solid #eee; border-radius: 8px;">
            Tidak ada order yang perlu validasi final
        </div>
        @endif
    </div>
@endsection
